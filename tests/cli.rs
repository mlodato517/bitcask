...
fn cli_access_server(engine: &str, addr: &str) {
    ...
    let handle = thread::spawn(move || {
        ...
        // Need to semi-gracefully stop `sled` so it writes things to disk. Use SIGTERM instead of
        // `child.kill()` which uses SIGKILL.
        let mut kill = Command::new("kill")
            .args(["-s", "TERM", &child.id().to_string()])
            .spawn()
            .unwrap();
        kill.wait().expect("server exited before killed");
    });
    thread::sleep(Duration::from_secs(1));
    ...
    // Give `sled` a second to give up the port. Without this, the next command to open the server
    // fails to bind the address because it's already in use.
    thread::sleep(Duration::from_secs(1));
